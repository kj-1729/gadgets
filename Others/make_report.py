...
class stat_node:
	def __init__(self):
		...
		self.branch1 = u'   ┣'.encode(self.encode_type)
		self.branch2 = u'   ┗'.encode(self.encode_type)
		self.branch3 = u'   ┃'.encode(self.encode_type)
		self.indent_0 = ''
		self.indent_1 = ' '
	...
	def print_tree_downward(self, delim, header, last_flag):
		...
		if last_flag == 1:
			...
			header_new = header + self.branch3
...
class stat_tree:
	...
	def print_id2node(self):
		for k, v in self.id2node.items():
			print k, v.get_id(), v.get_fullname()
