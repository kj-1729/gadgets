...
class mksql_ctable:
			...
			for loop in range(self.num_fields):
				if len(data[loop]) == 0:
					...
					if len(self.datatype[loop]) == 0:
						self.datatype[loop] = self.check_datatype(data[loop])
						...
						if self.datatype[loop] == 'datetime':
							...
							if self.is_date_num(data[loop]) == 'Y':
								pass
							elif self.is_number(data[loop]) == 'Y':
								self.datatype[loop] = 'number'
							else:
								self.datatype[loop] = 'char'
						elif self.datatype[loop] == 'number':
							if self.is_number(data[loop]) == 'Y':
								self.datatype[loop] = 'number'
							else:
								self.datatype[loop] = 'char'
	...
	def output_stats(self):
		...
		print "id\tlabel\tdatatype\tmax(val)\tmin(val)\tmax(len)\tmin(len)\tnum_null\date_format\n"
		for loop in range(self.num_fields):
			write(str(loop))
